...
# Link the VS Code extension into every extensions directory that exists, and record it there. A
# symlink rather than a copy, so editing the grammar and reloading the window is the whole edit
# loop. Every root gets it because which one is live depends on how the window was opened: a local
# window reads ~/.vscode, and a Remote-SSH window reads ~/.vscode-server on the machine it connected
# to.
#
# Linking alone is not enough. VS Code keeps `extensions.json` beside the extension folders as the
# record of what is installed, and writes it on install and uninstall rather than on scan, so a
# folder it does not list is a folder that does not exist and a version it lists that disagrees with
# the manifest is an extension stuck asking for a restart that fixes nothing. `register.py` is what
# keeps the two in step; without python3 the link still happens and the record has to be written by
# hand.
...
editor-install:
	...
	for root in $(EDITOR_ROOTS); do \
		...
		if command -v python3 >/dev/null 2>&1; then \
			python3 $(CURDIR)/editors/vscode/register.py "$$root/extensions" || true; \
		else \
			echo "no python3: add norn-lang.norn to $$root/extensions/extensions.json by hand" >&2; \
		fi; \
		found=1; \
	done; \
	if [ $$found -eq 0 ]; then \
