...
That a handle exposes only inputs and exports is also what lets a handle carry a trait. A reactor handle is an ordinary value — it copies, it passes as a parameter, it sits in a struct or a collection — so `impl Health for Gate` is allowed, and an impl written for one reaches exactly the surface every other holder of that handle reaches: the inputs it can `send` to and the signals it can read the latest of, never anything the reactor keeps private. The constraint that makes it safe is therefore not a new one; it is the boundary that was already there. Because `send` suspends, the useful half of this needs a trait member that is a `task fn`, so the two landed together (BOOTSTRAP §8 item 7). The handle's own member surface wins over any impl — `gate.opened()` names the input, not a method — and a method whose name would shadow an input or an export is refused where it is written, so a reader never has to know that the parentheses decide which of two unrelated things is meant. Dispatch stays static: a bound is not a type, so this is polymorphism over reactor *types*, not a collection holding differently-typed handles.
...
The floor this stands on is one compiler-provided atom: `Slots<T>`, a fixed-capacity typed slab — landed with `BOOTSTRAP.md` §8 item 5's sequence wave (2026-08-20). It is *copyable*, with the copy-on-write inside its own write ops — "Swift with safe ops": copying a slab is a refcount bump, and every write op does the storage-uniqueness check internally, mutating in place when the slab is the storage's only holder and cloning the storage once when a copy is watching, unobservably either way. Dropping one is fine (its memory is freed; there is no descriptor to leak), it may live in reactor state, and its element may not be affine — a freely-copied slab of descriptors would mint second owners. The earlier "no reference counts natively" survives as the native fast lane's elision promise: a slab the compiler can prove uniquely held needs no count consulted, an optimization the semantics never observes. Its operations are get, set, and take by index, bounds-trapped, with defined empty-slot semantics — an `Option` or a trap, never undefined behavior. "Raw memory" is a representation promise rather than a semantics change: a contiguous buffer natively, linear memory on wasm, an array on a TypeScript target — the same program, the same traps.

On that atom, the growable sequence is an ordinary struct holding a `Slots<T>` and a length — `std/buf`'s `Buf<T>`, landed the same wave: `push` writes through `mut`, growth allocates a larger slab and copies the written prefix across, and the user-facing type copies like any value, copy-on-write at the storage level. Hash maps follow as open addressing over `Slots<Option<(K, V)>>`. The storage-uniqueness test lives *inside* the ops rather than on the surface: under safe ops a user-facing `is_unique` is an optimization hint with no consumer, so its exposure is deferred — std-gated, when a consumer exists. No `unsafe` appears anywhere in this story, because what a standard library actually requires is not undefined behavior; it is a primitive with defined trapping and an honest cost model.
...
*v0 answers this with a theorem rather than a discipline, and the theorem survived loops by narrowing.* While the subset had no loop construct, recursion was the only way a pure function could fail to return; now that `while` and `loop` exist, the rule is that turn-reachable code may contain neither recursion nor loops — a direct guard in the turn body plus one pass over the reachable call graph, the same two-layer enforcement impurity already uses. Every turn is then a finite tree of calls over bounded expressions and provably terminates, with no annotation burden and no runtime budget; loops live in `fn`s and `task fn`s, which is where every server shape wants them anyway.

The direction the ban relaxes in, decided so the options above can stop being options: toward loops that are bounded by construction, never toward termination proofs. If the checker can be *certain* a loop ends, it is welcome in a turn; otherwise it is blocked — and certainty here means the bound is the data, not an analysis. A `for` over a finite collection cannot fail to end, so `for` is expected to be admissible in a turn when it arrives with the standard library, while `while` and `loop` stay barred there permanently: any general-termination check is conservative by the halting problem, and its acceptance boundary would become an implementation detail users hit as "not provably finite" with no actionable fix. Ranking functions and `decreases` annotations remain fallbacks only if a real turn ever genuinely needs a `while`, which nothing in the corpus suggests.
